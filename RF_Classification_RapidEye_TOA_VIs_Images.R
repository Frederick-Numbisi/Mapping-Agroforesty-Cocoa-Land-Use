...
img = brick("C:/........./BOKOA_REye_TOARef_Intensity.tif") # Create stack of bands from the image


#names(img) <- c(paste0("B", 1:4, coll = ""), "B5") # rename image bands as B1 to B5
names(img) <- c(paste0("B", 1:9, coll = ""), "B10") # rename the image bands (TOA and VIs) as B1 to B10

trainData = shapefile("........./Lsamples_all1.shp") # 
responseCol = "LUClass" # select the column name (in the dataframe) for the response variable


dfAll = data.frame(matrix(vector(), 0, length(names(img)) + 1)) # convert the dataframe to matrix

# Extract the data points from each land use/cover category or polygon class in the shapefile
...
# From explorative analysis, the LUClass 7 has just 44 pixels while other class range from 106 to 934 .
# So it is worthwhile conducting a stratified random sampling, other rf classification
# accuracy will be biased and more representatiive of the classes with high sampled pixels


# Convert all predictor band values from categorical to numeric
# Extracted values using the shapefiles and save the values at categories for each polygon/
...
#i <- sapply(dfAll, is.factor) # select only variables whose values are factors
#dfAll[i] <- lapply(dfAll[i], as.character) # convert the factor variable to characters

dfAll[] <- lapply(dfAll, function(x) as.numeric(as.character(x))) #convert all variables to numeric data type

dfAll$LUClass = as.factor(dfAll$LUClass) # Conver the Response variable to categorical data



#-----------------------------------------------------

#dfAll$Nsample = NA # create new variable N and assign its values to NO Data



# TRATIFIED RANDOM SAMPLING DESIGN
...
dfAll$LClass = NA # Assign NA to all values in LClass column
...
write.csv2(dfAll, 'C:/...../BOKOA_REye_TOARef_Intensity.csv') # Save the new dataframe in .csv file.
...
# Create a Stratified Random Sample (Dataframe) of 70% of the records for each land use class

p = 0.7 # probability of 70 % of original sample size

dsample = data.frame() # create empty sample dataframe

system.time(
  for(i in levels(dfAll$LClass)){
    dsub = subset(dfAll, dfAll$LClass == i) # create a subset each land use class
    B = ceiling(nrow(dsub) * p) # compute 30% of total number rows in each class
...
sdfAll = dsample # Copy the dataframe
names(sdfAll)
sdfAll = sdfAll[,-51] # Delete duplicate column or variable
summary(sdfAll)
...
# Evaluate Prediction and Confusion matrix based on the training data
...
plot(modFit_rf, main='OOB Error plot; RE2 Model')

#--------------------------------------------------------------------------
# Tunning the Random Forest based on Error Rate (plot of model)
# As the number of trees increase, the OOB error trend intially drops and then remains constant 
...
# Now tune the model using information from OOB error plot

# Tune mtry

t1 = tuneRF(sdfAll_train[,-51], sdfAll_train[,51],
            stepFactor = 0.2,
            plot = TRUE,
            ntreeTry = 450,
            trace = TRUE,
...
# C) Run classification based on final (best) model
#-------------------------------------
...
# Save classified map for use in other GIS

writeRaster(output, 'C:/..../BOKOA_REye_TOARef_Intensity_RFclassified.tif',
            format="GTiff", overwrite=TRUE)
writeRaster(output, 'C:/...../BOKOA_REye_TOARef_Intensity_RFclassified2.tif',
            format="GTiff", overwrite=TRUE)


# Plot a histogram of No. of Nodes for the RF model

hist(treesize(modFit_rf2),
     main = 'No. of Nodes for the Trees: RE2 RF Model',
     col = 'grey')

varImpPlot(modFit_rf2,
           sort = T,
           n.var = 30,
           main = 'Important RE3  Variables') # to see which variables play an important role in the model2
...
write.csv (var.imp_M2[order(var.imp_M2$MeanDecreaseGini,decreasing = T),], 'C:/......../RESULTS/BakoaSAR_RE2_Model_ImportantVar.csv')
...
prob_VH = predict(modFit_rf3, sdfAll_train, type='prob')
View(prob_VH)
write.csv(prob_VH, 'C:/......../RESULTS/BakoaSAR_RE2_ClassProbability_trainData.csv')
...
write.csv(importance(modFit_rf2, type=2), file="C:/....../RESULTS/BakoaSAR_RE2_AllVariable_ImportanceValues.csv")
write.csv(importance(modFit_rf2), file="C:/....../RESULTS/BakoaSAR_RE2_AllVariable_ImportanceValues1.csv")

# to find out which predictor variables are actually used in the random forest
write.csv(varUsed(modFit_rf2), file = "C:/...../RESULTS/BakoaSAR_RE2_PredictorVariables_Frequency.csv")


#-------------------------------------------------------------------------
# EXTRACT A SINGLE TREE FROM THE FOREST

#getTree(modFit_rf2, 325, labelVar = TRUE) # ectract tree 325

#-------------------------------------------------------------------------
# Multi-dimensional Scaling Plot of Proximity Matrix

#MDSplot(modFit_rf2, sdfAll_train$LClass)
#MDSplot(modFit_rf2, sdfAll_train$LClass, pch=as.numeric(sdfAll_train$LClass))

#MDSplot(modFit_rf2, sdfAll_train$LClass, k = 4)
...
write.csv(class_prob, file="C:/....../RESULTS/BakoaSAR_RE2_Predicted_TestData_class_prob.csv")
writeRaster(class_prob2, 'C:/......./RESULTS/BakoaSAR_RE2_PredictedClassProbabilityMap.tif',
            format="GTiff", overwrite=TRUE)
View(class_prob)
...
View(class_votes1)
write.csv(class_votes1, file="C:/....../RESULTS/BakoaSAR_RE2_Model_class_votes.csv")
...
View(class_prob4)
write.csv(class_prob4, file = 'C:/....../RESULTS/BakoaSAR_RE2_Predicted_AllData_Class_prob.csv')
...
View(output2) # object type is a layer with some NA values

writeRaster(output2, 'C:/...../RESULTS/BakoaSAR_RE2_PredictedProbabilityMap.tif',
            format="GTiff", overwrite=TRUE)
writeRaster(output2, 'C:/...../RESULTS/IMAGES4use/BakoaSAR_RE2_PredictedProbabilityMap2.tif',
            format="GTiff", overwrite=TRUE)


#------------------------------------------------------------------------------------
# Partial Dependence Plot
# It gives a graphical depiction of marginal effect of a variable on the class probability

# E.g. PDP For the variable (Sigma0_VH_db_mst_07Apr2015_Contrast) on class 1 - Cocoa Agroforest

#partialPlot(modFit_rf2, sdfAll_train, B19, '5',
#            main = 'VH: Partial Dependence of class 5 on Band50') 

#PDP For the variable (Sigma0_VH_db_mst_07Apr2015_Contrast) on class 6 - Secondary Forest

#partialPlot(modFit_rf2, sdfAll_train, B19, '6',
#            main = 'VH: Partial Dependence class 6 on Band50') 

#partialPlot(modFit_rf2, sdfAll_train, B19,
#            main = 'VH: Partial Dependence on Band50') 

#partialPlot(modFit_rf2, sdfAll_train, B19, '7',
#            main = 'VH: Partial Dependence of class 7 on Band50') 
